...
namespace Vy
{
	...
	template<typename T>
	concept IsScriptConcept = std::is_base_of_v<VyEntityScript, T>;
	...
	public:
		...
		VY_NODISCARD bool isValid()
		{
            if (!m_Scene)
            {
                VY_THROW_RUNTIME_ERROR("Scene is invalid!");
            }

			return m_Handle != kInvalidEntityHandle && registry().valid( m_Handle );
		}
        ...
        VY_NODISCARD EntityHandle handle() const { return m_Handle; }
		...
		{
            VY_ASSERT(isValid(), "Entity is invalid.");
         ...
         */
		template<IsScriptConcept T, typename... Args>
		T& add(Args&&... args)
		{
        ...
        T* tryGet()
		{
            VY_ASSERT(isValid(), "VyEntity is invalid.");
         ...
         */
        template <typename T>
        requires OptionalComponentConcept<T>
        void remove()
		{
