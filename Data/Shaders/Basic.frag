...
// ================================================================================================
// DESCRIPTOR SET 2-4 : SHADOW MAPS

layout (set = 2, binding = 0) uniform sampler2DArray   uCascadedShadowMap;
layout (set = 3, binding = 0) uniform samplerCubeArray uPointShadowCubeMap;
layout (set = 4, binding = 0) uniform sampler2DArray   uSpotShadowMap;

// ================================================================================================
// DESCRIPTOR SET 5 : MATERIAL TEXTURES

layout (set = 5, binding = 0) uniform sampler2D uAlbedoTexture;
layout (set = 5, binding = 1) uniform sampler2D uNormalTexture;
layout (set = 5, binding = 2) uniform sampler2D uMetallicRoughnessTexture;
    ...
    vec3 ambient)
{
    ...
    {
		if(-fragViewPos.z < uCascadedShadowPassUBO.CascadeSplits[i]) 
        {
			cascadeIndex = i + 1;
		}
	}
    ...
    if(shadowCoord.z > -1.0f && shadowCoord.z < 1.0)
	{
		...
		for(int y = -sampleRadius; y <= sampleRadius; y++)
		{
			...
			{
				float closestDepth = texture(uCascadedShadowMap, vec3(shadowCoord.xy + vec2(x, y) * pixelSize.xy/10, cascadeIndex)).r;
    ...
    float         lightIndex)
{
	...
	//if(currentDepth > -1.0f && currentDepth < 1.0)
    {
		...
		int sampleRadius = 20;
        vec3 pixelSize = 1.0 / textureSize(uSpotShadowMap, 0);
...
void main()
{
    ...
    float metallic  = texture(uMetallicRoughnessTexture, fragUV).b;
    ...
    // Point Light List
    for(float i = 0; i < uUbo.PointLightsCount; i++)
    {
    ...
    // Spot Light List
    for(float j = 0; j < uUbo.SpotLightsCount; j++)
    {
