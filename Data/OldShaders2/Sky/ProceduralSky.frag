...
void main()
{
    ...
    vec3  sunDir       = normalize(uPush.sunDirection.xyz);
    float sunIntensity = uPush.sunDirection.w;
    ...
    for (int i = 0; i < STEPS; ++i)
    {
        ...
        vec3 attenuation = exp(-(uPush.rayleigh * 1e-5 * vec3(5.8, 13.5, 33.1) * (opticalDepth.x + sunOpticalDepth.x) +
                                uPush.mie * 1e-5 * 21.0 * (opticalDepth.y + sunOpticalDepth.y)));
    ...
    vec3  rayleighColor = totalRayleigh * phaseRayleigh(mu) * uPush.rayleigh * 1e-5 * vec3(5.8, 13.5, 33.1);
    vec3  mieColor      = totalMie * phaseMie(mu, uPush.mieEccentricity) * uPush.mie * 1e-5 * 21.0;

    vec3 color = (rayleighColor + mieColor) * sunIntensity * uPush.sunColor.xyz;

    // Sun Disk
    // Calculate transmittance along the view ray (using the accumulated optical depth)
    vec3 extinction    = uPush.rayleigh * 1e-5 * vec3(5.8, 13.5, 33.1) * opticalDepth.x + uPush.mie * 1e-5 * 21.0 * opticalDepth.y;
    ...
    if (mu > 0.999)
    {
        ...
        color += uPush.sunColor.xyz * sunIntensity * transmittance * sunMask * 20.0;
    }
