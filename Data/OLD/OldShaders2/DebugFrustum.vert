...
vec3 getFrustumVertex(int index)
{
    ...
    float z = (cornerIndex < 4) ? uPush.NearZ : uPush.FarZ;
    float h = tan(uPush.FovY * 0.5) * z;
    float w = h * uPush.AspectRatio;

    float xSign = (cornerIndex == 0 || cornerIndex == 1 || cornerIndex == 4 || cornerIndex == 5) 
        ? 1.0 
        : -1.0;
    ...
    float ySign = (cornerIndex == 0 || cornerIndex == 3 || cornerIndex == 4 || cornerIndex == 7)
        ? -1.0
        : 1.0; 
    // Vulkan Y is down? No, standard camera space Y is down in Vulkan?
