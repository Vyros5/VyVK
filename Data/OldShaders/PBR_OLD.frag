...
//     float metalness = texture(metalnessTexture, fragUV).r;
//     float roughness = texture(roughnessTexture, fragUV).r;

//     // Outgoing light direction (vector from world-space fragment Position to the "eye").
//     vec3 Lo = normalize(ubo.InverseView[3].xyz - fragPosWorld);

//     // Get current fragment's normal and transform to world space.
//     vec3 N = normalize(2.0 * texture(normalTexture, fragUV).rgb - 1.0);
...
//     // outColor = vec4(directLighting + ambientLighting, 1.0);
//     outColor = vec4(texture(albedoTexture, fragUV).rgb, 1.0);
